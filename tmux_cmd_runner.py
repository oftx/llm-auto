...
def _execute_str(command_string: str, term_instance: 'TmuxTerminal') -> bool:
        ...
        for attempt in range(max_attempts):
            ...
            if output_lines:
                ...
                if any(char in last_line for char in ['$', '>', '➜', '#', '%']) and len(last_line) > 0:
                    time.sleep(0.1)  # 再等待一小段时间确保提示符完全加载
                    return True
        return False
    ...
    if exit_code == -1:
        ...
        choice = input(
            "    请检查 tmux 会话的实际运行情况，并决定是否继续执行？(y/N): ").lower().strip()
        if choice == 'y':
            print("    用户选择继续执行。")
            return True
        else:
...
class TmuxTerminal:
    ...
    def __enter__(self):
        self._session = self._server.find_where(
    ...
    def capture_clean_output(self) -> str:
        ...
        full_output: List[str] = self._pane.cmd(
            "capture-pane", "-p", "-S-", "-E-").stdout
